...
func CreateUser(username string, passphrase string, inviteCode string) (*User, error) {
	...
	cacheClearUserByUuid(user.Uuid)
	cacheClearUserByUsername(user.Username)
	return user, nil
}
...
func (u User) Save() error {
	...
	cacheClearUserByUsername(u.Username)
	cacheClearUserByUuid(u.Uuid)
...
func cacheClearUserByUsername(username string) {
	key := "user-by-username-" + username
	cache1m.Remove(key)
}

func cacheClearUserByUuid(uuid string) {
	key := "user-by-uuid-" + uuid
	cache1m.Remove(key)
}
